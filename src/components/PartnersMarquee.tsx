...
const PartnerItem = ({ partner }: { partner: typeof partners[0] }) => {
  if (partner.type === 'image' && partner.logo) {
    return (
      <div className="flex items-center justify-center h-20 md:h-24 px-8 flex-shrink-0">
        <img
          src={partner.logo}
          alt={partner.name}
  ...
  if (partner.type === 'badge') {
    return (
      <div className="flex items-center justify-center h-20 md:h-24 px-8 flex-shrink-0">
        <div className="flex items-center gap-2 px-6 py-3 bg-white rounded-lg shadow-md border border-gold/20">
          <span className="font-serif text-2xl font-bold text-gold">A+</span>
          <span className="font-sans text-sm font-semibold text-primary">BBB</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex items-center justify-center h-20 md:h-24 px-8 flex-shrink-0">
      <span className="font-serif text-xl md:text-2xl text-primary font-medium">
        {partner.name}
      </span>
    </div>
  );
...
export const PartnersMarquee = () => {
  return (
    <section className="py-12 md:py-20 bg-gradient-to-b from-background to-secondary/30 overflow-hidden">
